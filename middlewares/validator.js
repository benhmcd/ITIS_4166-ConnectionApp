...
exports.validateConnection = [body('name').notEmpty().withMessage('The name cannot be empty.').trim().escape(),
...
body('start_time').matches('^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$').withMessage('The start time must be a valid time.').notEmpty().withMessage('The start time cannot be empty.').trim().escape(),
body('end_time').matches('^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$').withMessage('The end time must be a valid time.').notEmpty().withMessage('The end time cannot be empty.').trim().escape().custom((value, { req }) => {
